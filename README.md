...
台灣裁判語義檢索服務（22,563,805 筆，截至 2026-09-02）,讓你能用自然語言搜尋判決,並將檢索結果帶入自己的 AI 工具使用。

## 資料涵蓋（統計截至 2026-09-02，取自 production 資料庫）
...
| 命令 | 行政規則 | 88,382 筆 | 90 個機關發布之解釋性規定與裁量基準（行政程序法 §159），對外不生法規範效力 |
...
| 資料類型 | 數量 | 說明 |
|---|---:|---|
| 裁判書（司法院各級法院） | 22,563,805 筆 | 語義檢索＋詞彙檢索＋案號精確調卷；每日增量同步司法院公開資料，含事後更正與下架之同步對齊 |
...
| 資料類型 | 數量 | 說明 |
|---|---:|---|
| 審級關聯（上訴鏈） | 4,537,220 筆 | 隨判決附 `case_history`，含主文「廢棄／駁回」旗標 |
| 函釋效力履歷 | 69,461 筆 | 廢止／停止適用／被取代狀態追蹤，引用前驗效力 |
...
### 行政規則發布機關明細（90 個機關，統計日 2026-09-02）
...
- **22,563,805 筆**台灣裁判資料（截至 2026-09-02），經過結構化處理與向量化。
